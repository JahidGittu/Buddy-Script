  ...
  return (
    <div className="xl:col-span-3 lg:col-span-3 md:col-span-12 sm:col-span-12">
      <div className="flex flex-col h-[calc(100vh-75px)] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] flex-1 pt-[18px]">
        {/* Explore Section */}
        <div className="mb-4">
          <div className="bg-white rounded-lg pt-6 pb-1.5 px-6 mb-4 transition-all duration-200">
            ...
            <ul className="space-y-6">
              {exploreItems.map((item, index) => (
                <li key={index} className={`flex items-center ${item.badge ? 'justify-between relative' : ''}`}>
                  <a 
                    href={item.href} 
                    className="flex items-center text-gray-600 hover:text-blue-500 transition-colors duration-200 w-full text-base font-medium"
                  >
                    <span className="mr-3.5 w-5 h-5 flex items-center justify-center">
                      {item.icon}
                    </span>
                    {item.label}
                  </a>
                  {item.badge && (
                    <span className="text-white bg-[#0ACF83] border-2 border-white rounded-lg w-9 h-6 flex items-center justify-center text-xs font-normal absolute right-0">
                      {item.badge}
                    </span>
                  )}
                </li>
        ...
        <div className="mb-4">
          <div className="bg-white rounded-lg pt-6 pb-1.5 px-6 mb-4 transition-all duration-200">
            <div className="flex items-center justify-between mb-6">
              <h4 className="text-lg font-semibold text-gray-900">Suggested People</h4>
              ...
              <div key={index} className="flex items-center justify-between mb-6 flex-wrap">
                <div className="flex items-center flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <a href={person.href}>
                      <h4 className="text-sm font-medium text-gray-900 leading-tight">{person.name}</h4>
                    </a>
                    <p className="text-xs font-light text-gray-900 leading-relaxed">{person.role}</p>
                  </div>
                </div>
                <div>
                  <a 
                    href="#0" 
                    className="bg-white border border-gray-300 rounded text-gray-500 text-xs font-medium px-2 py-1.5 block hover:bg-blue-500 hover:text-white hover:border-blue-500 transition-colors duration-200"
                  >
                    Connect
                  </a>
                </div>
        ...
        <div className="mb-4">
          <div className="bg-white rounded-lg pt-6 pb-1.5 px-6 transition-all duration-200">
            <div className="flex items-center justify-between mb-6">
              ...
              <a href="/events" className="text-blue-500 text-sm font-medium hover:underline block cursor-pointer py-1 px-0.5">
                See all
              </a>
            </div>
            ...
            {events.map((event, index) => (
              <div key={index} className="block mb-4 last:mb-0">
                <div className="bg-white shadow-card rounded-lg cursor-pointer">
                  <a href={event.href} className="block">
                    ...
                    <div className="p-4">
                      <div className="flex items-center">
                        <div className="bg-[#0ACF83] rounded-sm px-2 py-2 text-center min-w-[60px]">
                          <p className="font-bold text-lg text-white leading-tight">{event.date.day}</p>
                          <p className="text-lg font-normal text-white leading-tight">{event.date.month}</p>
                        </div>
                        <div className="pl-2 flex-1 min-w-0">
                          <h4 className="text-base font-medium text-black leading-relaxed line-clamp-2">
                            {event.title}
                          </h4>
                        </div>
                      </div>
                  ...
                  <div className="p-4 pt-0">
                    <div className="flex items-center justify-between pt-1 pb-1">
                      <p className="text-xs font-medium text-gray-500 opacity-70">{event.attendees}</p>
                      <button 
                        className="text-blue-500 text-xs font-medium bg-blue-50 border border-blue-500 rounded px-3.5 py-1 hover:bg-blue-500 hover:text-white transition-colors duration-200"
                      >
                        Going
                      </button>
                    </div>
